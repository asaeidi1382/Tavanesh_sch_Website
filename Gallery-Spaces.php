...
<head>
  ...
  <title>گالری فضاهای مدرسه - دبیرستان توانش</title>
  ...
  <style>
    ...
    .btn:hover { background: var(--turquoise-100); }

    .gallery {
      display: grid;
      grid-template-columns: repeat(3, 1fr); /* پیش‌فرض سه ستونه */
      gap: 14px;
      padding: 16px;
      max-width: 1400px;
      margin: 0 auto 40px;
...
<main class="content">
  ...
  <script>
    // مسیر و نام‌گذاری برای فضاهای مدرسه
    const basePath = "images/SchoolSpaces/";
    const startIndex = 1;
    const endIndex = 30; // تعداد تصاویر این گالری را اینجا تنظیم کن

    // نام فایل‌ها با صفرهای پیشرو: Tavanesh-SchoolSpaces-001.jpg
    function pad3(n){ return n.toString().padStart(3, '0'); }
    function fileName(n){ return `Tavanesh-SchoolSpaces-${pad3(n)}.jpg`; }
    ...
    // کلیک روی پس‌زمینه‌ی لایت‌باکس = بستن
